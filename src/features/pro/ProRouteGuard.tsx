 ...
 * Mounting inside the screen keeps the locked/unlocked decision
 ...
 * `resolveGateState` (the pure gate, binary post-cutover). While the
 * state is `'locked'` — either because the bootstrap is still loading
 * or because the user is genuinely free — it renders `<ProLock />`.
 * When the state flips to `'unlocked'` it renders `children` instead,
 * so a successful purchase / restore re-renders the screen with the
 * real content without remounting the route.
 *
 * Post-cutover (0039, slice B): the `'frozen'` branch is gone (gate is
 * binary). The pre-cutover `'frozen'` lock surface (a danger-toned
 * card with a "trial expired" message + upgrade CTA) is replaced by
 * the standard `<ProLock />` — the trial pill + intro caption (slice
 * C) handle the trial-aware UX separately.
 */
 ...
 * (loading or free), renders `<ProLock />`. Once the user becomes Pro,
 * renders `children` in place. The route is still mounted in all
 * states — the children are simply not rendered while locked — so
 * navigation history is preserved and a successful purchase animates
...
export function ProRouteGuard({
  ...
  const state = resolveGateState(isPro, isLoading);
  if (state === 'locked') {
    return (
      <ProLock
