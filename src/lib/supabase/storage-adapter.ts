...
export function createSecureStoreAdapter(
  ...
  /**
   * Per-key mutation queue. supabase-js can drive overlapping writes (an
   * auto-refresh rewriting the session while a sign-out clears it); without
   * serialization their chunk writes interleave, the two meta commits race,
   * and the survivor can point at a torn mix of chunks — getItem then rejects
   * the value and the session is lost. Reads stay unqueued: a read during a
   * write is already safe because getItem validates length + content hash and
   * returns null on a tear (never garbage).
   */
  const queues = new Map<string, Promise<void>>();

  const enqueue = (key: string, task: () => Promise<void>): Promise<void> => {
    // Chain onto the previous task REGARDLESS of its outcome — a failed write
    // must not block subsequent ones. The stored link swallows rejections so
    // the chain stays alive; the returned promise still propagates them to the
    // caller of THIS call.
  ...
  const doGetItem = async (key: string): Promise<string | null> => {
  ...
  const doRemoveItem = async (key: string): Promise<void> => {
    const meta = parseMeta(await backend.getItemAsync(metaKey(key)));
  ...
  const doSetItem = async (key: string, value: string): Promise<void> => {
    if (value.length === 0) {
      await doRemoveItem(key);
      return;
    }
  ...
  return {
    getItem: (key) => doGetItem(key),
    setItem: (key, value) => enqueue(key, () => doSetItem(key, value)),
    removeItem: (key) => enqueue(key, () => doRemoveItem(key)),
  };
}
