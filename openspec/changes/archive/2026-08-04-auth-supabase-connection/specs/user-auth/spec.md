...
The system MUST sign the user out on demand, MUST clear the persisted session, and MUST route the user back to the sign-in screen. The profile screen MUST expose a sign-out control to the signed-in user. When the server-side revoke fails (offline), the local session is still cleared and the user is signed out on the device: the system MUST NOT surface a misleading failure. Only a sign-out that leaves the local session intact is a genuine failure worth surfacing.
