...
- [ ] T4.1 Acceptance gates — AUTOMATED SUBSET ✅ PASSED (verify-report #1196, main @ 7b61930/9919c82): `pnpm typecheck` exit 0; `pnpm test:run-rate` 15/15 green; `pnpm test:run-rate-hook` 8/8 green (real hook mount, deterministic clock, recalc-once + last-good + mid-session navigation pins); Feb-vs-31d (spec gate 4) ✅ automated (case 11). MANUAL SUBSET ⏳ PENDING user (4 gates, do NOT mark complete until run): (1) mid-month hand-check — MTD, % vs same-days last month, ~projection vs hand-computed cache values (spec gate 2); (2) 5 hidden states visual — day 1–2, empty month, past month, no baseline, read failure, card absent on device (spec gate 3); (3) cache-miss → recalc → render on device (spec gate 5, hook-level automated); (4) tap → Analytics (spec gate 6, no test exists).