...
- [x] 5.1 Unit test `transformCacheToCategoryTotals` with fixture cache rows (valid row, empty row, missing categories). ✅ Done — `scripts/test-monthly-cache.mjs` created with 8 unit tests covering: null row, empty category_totals, field mapping, percent_of_total normal/0/rounding, descending order. Wired into `package.json:test:monthly-cache` via `scripts/tsconfig.monthly-cache-test.json`. Stub at `scripts/test-stubs/auth.ts` (useSessionUser with `__setUserId` seam).
- [x] 5.2 Integration test `useMonthlyCache` cache-hit path (mock Supabase returns row) and cache-miss path (mock returns null, verify mutation fires then refetch). ✅ Done — `scripts/test-monthly-cache.mjs` includes 7 integration tests that mount the real `useMonthlyCache` hook with jsdom + react-dom + act + real react-query (`QueryClientProvider`). Cache-hit: verifies no recalc + no refetch. Cache-miss: verifies `triggerMonthlyRecalc` fires + refetch populates totals. Household mode: uses `readCategoryTotals`, no recalc. Mock controlable via `scripts/test-mocks/feature-access.js`. Not covered: error/unconfigured read status branch and no-userId guard (`enabled: false`).
