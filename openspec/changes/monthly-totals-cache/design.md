...
- [ ] Weekly chart is broken (same pagination bug) but deferred — should we address it in this change or create a follow-up?
- [ ] Backfill: existing users have historical months with no cache rows — the cache-miss fallback handles this, but the first render of old months triggers an RPC. Acceptable?
