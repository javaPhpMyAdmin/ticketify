...
> Source: change `delete-account` (archived 2026-09-17). Merged from delta `openspec/changes/archive/2026-09-17-delete-account/specs/user-auth/spec.md`.
...
After the server-side deletion primitive succeeds, the client MUST perform the same local cleanup chain that a normal sign-out would trigger (`queryClient.clear()`, `useReceiptsStore.resetAll()`, `useProStore.reset()`, optional `useHouseholdStore.reset()`, `useSessionStore.session = null`) and MUST route the user to `/sign-in` (the codebase route that backs the `(auth)` route group in Expo Router). The existing `SIGNED_OUT` event listener does NOT fire on hard delete because `supabase.auth.signOut()` is never called; the client MUST therefore run the cleanup chain manually rather than rely on the listener (see `REQ-ACCTDEL-10` of `user-account-deletion`).
...
- AND the user is still routed to `/sign-in`
