...
> **Archived 2026-09-19** from change `legal-compliance`. All 5 requirements (REQ-1 acceptance record, REQ-2 current-version gate, REQ-3 sign-up checkbox, REQ-4 deferred flush, REQ-5 blocking gate) and the 4 acceptance gates are implemented and verified. See `openspec/changes/archive/2026-09-19-legal-compliance/archive-report.md` for the merge history and the open follow-ups (legal copy DRAFT pending legal review; live PostgREST client→definer contract check noted as a U4 open gap).
