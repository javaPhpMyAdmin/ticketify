 ...
 *
 * `common:back` resolves to the es-AR back label (the header
 * accessibility-label contract the back-button test pins).
 */
...
/** Swaps the active legal bundle + locale the stub resolves against. */
...
export function useTranslation(_namespaces: string[] | string) {
  const t: TFunction = (key, options) => {
    ...
    if (ns === 'common') return COMMON_TRANSLATIONS[dotted] ?? key;
    if (ns === 'legal') {
